 ...
 * If 'physical_address' is non-null, stores the physical address of the allocation.
 */
uint32_t kmalloc_(uint32_t size, int align, uint32_t *physical_address)
{
...
/**
 * Allocate 'size' bytes page-aligned, and return physical address via out-parameter.
 */
...
/**
 * Simple allocator: allocate 'size' bytes without alignment or physical address.
 */
...
static int32_t find_smallest_hole(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    while (i < heap->index.size) {
        ...
        if (hole_size >= (int32_t)size + (int32_t)(sizeof(header_t) + sizeof(footer_t))) break;
        i++;
    }
...
void *alloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
    ...
    header_t *orig = (header_t*)orderedArray_lookup(idx, &heap->index);
    uint32_t orig_pos = (uint32_t)orig;
    uint32_t orig_size = orig->size;

    orderedArray_remove(idx, &heap->index);
    uint32_t offset = page_align ?
        (PAGE_SIZE - ((orig_pos + sizeof(header_t)) % PAGE_SIZE)) % PAGE_SIZE : 0;
    if (offset) {
        orig_pos += offset;
        orig_size -= offset;
    }
    ...
    if (split > sizeof(header_t) + sizeof(footer_t)) {
        ...
        hole->size = split;
        hole->is_hole = 1;
        footer_t *hole_footer = (footer_t*)((uint32_t)hole + split - sizeof(footer_t));
