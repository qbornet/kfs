...
static inline void putnbr_hex_upper(unsigned long nbr, int *ret)
{
...
static inline void putnbr_hex(unsigned long nbr, int *ret)
{
    for(int shift = 28; shift >= 0; shift -= 4) {
        uint8_t nibble = (nbr >> shift) & 0xF;
        if(!nibble) continue;
...
static inline void print_hex(char c, unsigned long nbr, int *ret)
{
    if(c == 'X') {
...
static inline void print_udec(unsigned long nbr, int *ret)
{
    ...
    terminal_putchar(nbr % 10 + '0');
}

static inline void print_dec(long nbr, int *ret)
{
    unsigned int nb = 0;
