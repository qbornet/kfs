...
{
    uint32_t len = FRAME_INDEX(nframes);
    for (uint32_t i = 0; i < len; i++) {
        uint32_t word = frames[i];
        if (word != 0xFFFFFFFF) {
...
void initialise_paging()
{
    ...
    nframes = FRAME_NUMBER(mem_size);
    frames = (uint32_t *)kmalloc(FRAME_INDEX(nframes));
    memset((uint8_t *)frames, 0, FRAME_INDEX(nframes));
